...
<body>
	...
	List<Cart> listOfCart = cartController.getCartListByUserId(user.getUser_Id());
			...
			<table class="table table-hover">
				...
				<tbody>
					...
					for (Cart c : listOfCart) {
						Product prod = productController.getProductsByProductId(c.getProduct_Id());						
					%>
					<tr class="text-center">
						...
						<td><a
							href="CartOperationServlet?cid=<%=c.getCart_Id()%>&opt=1"
						...
						<div class="qty"><%=c.getQuantity()%></div>
							<%if(c.getQuantity() > 1){ %>
							<a href="CartOperationServlet?cid=<%=c.getCart_Id()%>&opt=2"
						...
						<td><a
							href="CartOperationServlet?cid=<%=c.getCart_Id()%>&opt=3"
